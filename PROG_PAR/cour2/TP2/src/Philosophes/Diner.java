...
class Fourchette {
    ...
    synchronized void prendre() {
        while (!libre) {
            try {
                wait();
            } // Le philosophe s'endort sur la fourchette!
...
class Philosophe extends Thread {
    ...
    public void run() {
        while (true) {
            ...
            boolean deuxFourchettes = false;

            while (!deuxFourchettes) {
                fg.prendre();
                if (fd.tenter()) {
                    deuxFourchettes = true;
                } else {
                    fg.lacher();
                    System.out.println("LACHER ! ");
                    try {
                        Thread.sleep(1000);
