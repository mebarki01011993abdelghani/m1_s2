...
public class Line implements Runnable {
    ...
    private Line(CountDownLatch latch) {
        this.indiceY = Mandelbrot.taille;
        this.latch = latch;
    }

    private Line(int indiceX, CountDownLatch latch) {
    ...
    private void calculerLigne(Line ligne) {
    ...
    private static void buildExecutor() {
        ...
        for (int i = 0; i < nbThreads; i++) {
            executor.add(new Thread(new Line(latch))); // Thread build, en attente de la methode start
        }
    }
